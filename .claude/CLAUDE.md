...
- **OAuth scope changes are inert until the app is revoked** - `login()` requests `gist` only (#764). GitHub keeps whatever scopes an existing authorization already granted, so narrowing the request changes nothing for a live session. To actually apply it: revoke the app in GitHub settings and sign in again - and there are **two** OAuth apps, production and preview, so that is two revocations and two sign-ins. (Re-syncing preview from production afterwards is ordinary hygiene, not a consequence: `logout()` clears the stored gist id, but `findOrCreateGist` pins the preview gist rather than searching, so nothing is lost.) Expect the first load after revoking to look signed-in but read-only: `loadData` 401s and silently falls back to `loadPublicData()`, while the cached user keeps the nav avatar up (`isLoggedIn()` is just `!!this.token`). It only announces itself on the first write, and unevenly - a card edit says "Your session has expired", but toggling a card owned, the likelier first write, shows only a generic "Sync failed" chip.
