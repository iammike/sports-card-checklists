...
`;if(s?d+=`
        ...
        `,this.config.categories){const h=g=>{const c=g.match(/#[0-9a-fA-F]{6}/g)||[];if(c.length===0)return"";const y=c[0],f=parseInt(y.slice(1,3),16)/255,C=parseInt(y.slice(3,5),16)/255,b=parseInt(y.slice(5,7),16)/255,_=S=>S<=.03928?S/12.92:Math.pow((S+.055)/1.055,2.4);return .2126*_(f)+.7152*_(C)+.0722*_(b)>.4?" color: #1a1a1a;":" color: #ffffff;"};this.config.categories.forEach(g=>{if(g.gradient){const c=g.children?`.group-header.cat-${g.id}`:`.section-header.cat-${g.id}`;d+=`${c} { background: ${g.gradient};${h(g.gradient)} }
`}g.children&&g.children.forEach(c=>{c.gradient&&(d+=`.section-header.cat-${c.id} { background: ${c.gradient};${h(c.gradient)} }
`)})})}this.config.customCss&&(d+=this.config.customCss),document.getElementById("dynamic-theme").textContent=d}_ensureContrast(e,t,r){const i=h=>[parseInt(h.slice(1,3),16),parseInt(h.slice(3,5),16),parseInt(h.slice(5,7),16)],s=(h,g,c)=>`#${h.toString(16).padStart(2,"0")}${g.toString(16).padStart(2,"0")}${c.toString(16).padStart(2,"0")}`,n=([h,g,c])=>{const y=f=>(f/=255,f<=.03928?f/12.92:Math.pow((f+.055)/1.055,2.4));return .2126*y(h)+.7152*y(g)+.0722*y(c)},a=(h,g)=>(Math.max(h,g)+.05)/(Math.min(h,g)+.05),o=n(i(t));let[l,u,p]=i(e);const m=o<.5,d=m?1.18:.85;for(let h=0;h<25;h++){if(a(n([l,u,p]),o)>=r)return s(l,u,p);m?(l=Math.min(255,Math.round(l*d+(255-l)*.1)),u=Math.min(255,Math.round(u*d+(255-u)*.1)),p=Math.min(255,Math.round(p*d+(255-p)*.1))):(l=Math.round(l*d),u=Math.round(u*d),p=Math.round(p*d))}return s(l,u,p)}_setPageMeta(){document.title=this.config.title||"Checklist",document.getElementById("page-title").textContent=this.config.title||"";const e=document.getElementById("page-subtitle");this.config.subtitle?(e.textContent=this.config.subtitle,e.style.display=""):(e.textContent="",e.style.display="none");const t=document.getElementById("page-intro");this.config.introHtml&&(t.innerHTML=`<div class="intro-text">${this.config.introHtml}</div>`);const r=document.getElementById("total-label");this.config.totalLabel&&(r.textContent=this.config.totalLabel)}_isFlat(){return this.config.dataShape==="flat"}_getDefaultCategory(){if(this._isFlat())return null;const e=this.config.categories||[],t=e.find(r=>r.isMain!==!1)||e[0];return t?t.children&&t.children.length>0?t.children[0].id:t.id:null}_migrateDataShape(e){const t=this.config.dataShape||"categories",r=e.dataShape||"categories";if(t===r)return!1;if(r==="flat"){const i=[];(this.config.categories||[]).forEach(n=>{n.children&&n.children.length>0?n.children.forEach(a=>{(this.cards[a.id]||[]).forEach(o=>i.push(o))}):(this.cards[n.id]||[]).forEach(a=>i.push(a))}),this.cards=i,this.cardData={cards:i}}else{const i={};(e.categories||[]).forEach(o=>{o.children&&o.children.length>0?o.children.forEach(l=>{i[l.id]=[]}):i[o.id]=[]});const s=e.categories||[],n=s.find(o=>o.isMain!==!1)||s[0],a=n?.children?.[0]?.id||n?.id;a&&this.cards.length>0&&(i[a]=[...this.cards]),this.cards=i,this.cardData={categories:i}}return!0}getCardId(e){return this.config.cardDisplay?.includePlayerInCardId?btoa((e.player||"")+(e.set||"")+(e.num||"")+(e.variant||"")).replace(/[^a-zA-Z0-9]/g,""):this.checklistManager.getCardId(e)}isOwned(e){return this.checklistManager.isOwned(e)}getPrice(e){return e.price||0}getPriceThresholds(){return this.config.cardDisplay?.priceThresholds||{mid:3,high:10}}_playerNameHtml(e){const t=e.position?` <span class="player-position">${sanitizeText(e.position)}</span>`:"";return`<div class="player-name">${sanitizeText(e.player)}${t}</div>`}_subtitleLinesHtml(e){const t=this.config.customFields||{},r=Object.entries(t).filter(([s,n])=>n.position==="bottom"&&e[s]);if(r.length===0)return"";let i="";return r.forEach(([s,n])=>{const a=this._ensureContrast(n.color||"#888888",this._cardBg||"#ffffff",4.5),o=parseInt(a.slice(1,3),16),l=parseInt(a.slice(3,5),16),u=parseInt(a.slice(5,7),16),p=n.pill?`;background:rgba(${o},${l},${u},0.12)`:"",m=n.pill?" pill":"";i+=`<div class="card-subtitle-line${m}" style="color:${a}${p}">${sanitizeText(e[s])}</div>`}),i}createCardElement(e){const t=this._renderedCards.length;this._renderedCards.push(e);const r=e.collectionLink?null:this.getCardId(e),i=r?this.isOwned(r):!1,s=this.getPrice(e),n=this.config.cardDisplay?.showPlayerName!==!1&&e.player,a=e.player?e.player:this.config.cardDisplay?.showPlayerName===!1&&(this.config.searchPrefix||this.config.title)?this.config.searchPrefix||this.config.title:"",o=encodeURIComponent(CardRenderer.buildDefaultSearch(e,a)),l=CardRenderer.getEbayUrl(e.search||o),u=e.priceSearch||o,p=CardRenderer.getScpUrl(u),m=this.getPriceThresholds(),d=(e.type||"").replace(/\s*RC\b/gi,"").replace(/\bBase\b/gi,"").trim(),h=e.variant||"";if(e.collectionLink)return this._renderCollectionLinkCard(e,t);if(e.noCard){let f=`<div class="card no-card" data-card-idx="${t}">`;return f+='<div class="card-image-wrapper">',f+=CardRenderer.renderNoCardBadge(this.config.noCardLabel),f+="</div>",n&&(f+=this._playerNameHtml(e)),f+=this._subtitleLinesHtml(e),f+='<div class="card-actions links-only">',f+=CardRenderer.renderSearchLinks(l,p),f+="</div>",f+="</div>",f}let c=`<div class="${`card ${i?"owned":""}`.trim()}" id="card-${r}" data-card-idx="${t}" data-price="${s}"${e.sport?` data-sport="${e.sport}"`:""}${e.era?` data-era="${e.era}"`:""} data-type="${e.type||""}">`;if(c+='<div class="card-image-wrapper">',c+=CardRenderer.renderAttributeBadges(e,this.config.customFields),c+=CardRenderer.renderPriceBadge(s,m),c+=CardRenderer.renderCardImage(e.img,e.set,l),c+="</div>",n&&(c+=this._playerNameHtml(e)),c+=this._subtitleLinesHtml(e),e.set){let f=sanitizeText(e.set);if(e.num){const C=e.num.startsWith("#")?e.num:"#"+e.num;f+=` <span class="card-number">${sanitizeText(C)}</span>`}c+=`<div class="card-title">${f}</div>`}h&&(c+=`<div class="card-variant">${sanitizeText(h)}</div>`),d&&(c+=`<div class="card-type">${sanitizeText(d)}</div>`);const y=this.checklistManager.isReadOnly;return c+=`<div class="card-actions${y&&!i?" links-only":""}">`,c+=CardRenderer.renderOwnedControl(r,i,y),c+=CardRenderer.renderSearchLinks(l,p),c+="</div>",c+="</div>",c}_renderCollectionLinkCard(e,t){const r=sanitizeText(e.collectionLink);let i="";const s=e.collectionLink.match(/[?&]id=([^&]+)/),n=s?s[1]:null,a=n?(this._linkedStats||{})[n]:null;a&&typeof a.owned=="number"?i=`<span class="collection-badge">${a.owned} / ${a.total} CARDS</span>`:e.cardCount&&(i=`<span class="collection-badge">${e.cardCount} CARDS</span>`);let o;return e.stackImages&&e.stackImages.length>0?o=`<div class="card-stack">${e.stackImages.map(u=>`<img src="${sanitizeText(u)}" alt="" loading="lazy">`).join("")}</div>`:o=CardRenderer.renderCardImage(e.img,e.player,r),`<div class="card collection-link" data-card-idx="${t}" onclick="window.location.href='${r}'">
